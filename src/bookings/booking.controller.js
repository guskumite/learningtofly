...
import {
  hasDuplicateSeatNumber,
  validRepeatSeat,
} from "./utils/seat-availability-validator.js";
...
export const createBooking = catchAsync(async (req, res, next) => {
  ...
  const seatNumbersFlighs = await ticketService.findAllTicketsByFlightId(
    bookingData.dataBooking.flightId
  );
  //6. validar si alguno de los asientos que se estan intentando vender ya han sido vendidos antes
  const dontHasAvailavility = hasDuplicateSeatNumber(
    bookingData.tickets,
    seatNumbersFlighs
  );
  ...
  //TODO: Modificar tabla de aviones para indicar los numeros de asientos que tienen los aviones y aca hacer una validacion para saber si el asiento que se desea ingresar existe en ese avion
